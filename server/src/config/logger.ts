...
function cleanOldLogs() {
  const now = Date.now();
  if (now - lastCleanTime < 12 * 60 * 60 * 1000) return;
  lastCleanTime = now;
...
function appendToFile(level: LogLevel, message: string, meta?: any) {
  try {
...
function formatConsoleMessage(level: LogLevel, message: string, meta?: any): string {
  const timestamp = new Date().toISOString();
