...
export class PaymentService {
   ...
   */
  static async createPaymentOrder(userId: string, couponCode?: string) {
    // 1. Fetch user's cart
    ...
    for (const item of cart.items) {
      ...
      if (product.stock < item.quantity) {
        throw new AppError(
          `Insufficient stock for "${product.name || 'item'}".`,
          400
        );
      }
    ...
    if (couponCode) {
      ...
      if (!coupon.isValid(subtotal)) {
        throw new AppError(
          "Coupon code is inactive, expired, or minimum spend requirements are not met.",
          400
        );
      }
   ...
   */
  static async verifyPaymentSignature(userId: string, input: VerifySignatureInput) {
    const {
      razorpay_order_id,
      razorpay_payment_id,
      razorpay_signature,
