...
// ─── Add Product To Cart ─────────────────────────────────────────────────────
export const addToCart = catchAsync(async (req, res, _next) => {
    const userId = getUserId(req);
    const { productId, quantity, selectedColor } = req.body;
    const cart = await CartService.addToCart(userId, productId, quantity, selectedColor);
    res.status(200).json({
        success: true,
...
export const updateQuantity = catchAsync(async (req, res, _next) => {
    const userId = getUserId(req);
...
export const removeFromCart = catchAsync(async (req, res, _next) => {
    const userId = getUserId(req);
    const productId = req.params.productId;
