...
export const uploadProductImages = catchAsync(async (req, res) => {
    ...
    const uploadPromises = files.map((file) => UploadService.uploadBuffer(file.buffer, "products"));
 ...
 */
export const reorderProductImages = catchAsync(async (req, res) => {
    const productId = req.params.id;
    ...
    const submittedIds = new Set(images.map((img) => img.publicId));
    const currentIds = new Set(product.images.map((img) => img.publicId));
    if (submittedIds.size !== currentIds.size || [...submittedIds].some((id) => !currentIds.has(id))) {
 ...
 */
export const setPrimaryProductImage = catchAsync(async (req, res) => {
    const productId = req.params.id;
 ...
 */
export const uploadImagesGeneric = catchAsync(async (req, res) => {
    const files = req.files;
    if (!files || files.length === 0) {
        throw new AppError("Please select at least one image file to upload.", 400);
    }
