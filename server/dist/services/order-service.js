...
export class OrderService {
        ...
        while (attempts < maxRetries) {
            ...
            try {
                ...
                for (const item of cart.items) {
                    const product = await Product.findOneAndUpdate({ _id: item.product, stock: { $gte: item.quantity } }, { $inc: { stock: -item.quantity } }, { new: true, session });
                    if (!product) {
                        ...
                        throw new AppError(`Insufficient stock for "${exists.name}". Available: ${exists.stock}, Requested: ${item.quantity}.`, 400);
                    }
                ...
                if (input.couponCode) {
                    ...
                    if (!coupon.isValid(subtotal)) {
                        throw new AppError(`Coupon code is inactive, expired, or minimum spend requirements are not met (Min spend: ₹${coupon.minSpend}).`, 400);
                    }
     ...
     */
    static async getOrderDetails(userId, userRole, orderId) {
     ...
     */
    static async cancelOrder(userId, userRole, orderId) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            ...
            if (!["Pending", "Processing"].includes(order.orderStatus)) {
                throw new AppError(`Cannot cancel order. It is already in "${order.orderStatus}" status.`, 400);
            }
     ...
     */
    static async adminUpdateOrderStatus(orderId, newStatus) {
        const session = await mongoose.startSession();
        session.startTransaction();
        try {
            const order = await Order.findById(orderId).session(session);
    ...
    static async sendNewOrderEmails(userId, order) {
        try {
    ...
    static async sendCancelledOrderEmail(order) {
        try {
    ...
    static async sendStatusUpdateEmail(order, oldStatus, newStatus) {
        try {
            const user = await mongoose.model("User").findById(order.user);
            if (!user)
                return;
