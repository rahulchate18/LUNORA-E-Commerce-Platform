...
export class CartService {
     ...
     */
    static async addToCart(userId, productId, quantity, selectedColor) {
        const product = await Product.findById(productId);
        if (!product) {
            throw new AppError("Product not found.", 404);
        }
        if (product.stock < quantity) {
            throw new AppError(`Insufficient stock. Only ${product.stock} units are available.`, 400);
        }
        ...
        if (itemIndex > -1) {
            ...
            if (product.stock < newQty) {
                throw new AppError(`Insufficient stock. Adding ${quantity} items would exceed available stock (${product.stock}).`, 400);
            }
